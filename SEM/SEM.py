...
def sem_results(optimiser):
    ...
    model_spec = """
    # measurement model
flood_proneness =~ Inundation + assam_dist_from_major_rivers_updated_3857 + sum + GCN250_ARCIII_average + strm_filled_slope_degrees + ndvi + srtm_filled_dem + gmted_drainage_density_without_1
...
DEFINE(ordinal) Inundation Count_SDRF Count_relief Count_new Count_Erosion Count_Road Count_repair Count_IM Sum_SDRF Sum_relief Sum_new Sum_Erosion Sum_Roads Sum_repair Sum_IM damage_POPULATION_AFFECTED damage_humanliveslost damage_animalsaffectedtotal damage_animalsaffectedpoultry damage_animalsaffectedbig damage_animalsaffectedsmall damage_animals_washed_total damage_animals_washed_poultry damage_animals_washed_big damage_animals_washed_small damage_Houses_damaged_fully damage_Houses_damaged_partially damage_croparea_AFFECTED Embankment Other Road Bridge
...
Count_Erosion ~~ Sum_Erosion"""
    ...
    coeff_df.to_csv(cwd+r'Results/'+'Estimates_'+optimiser+'_SLSQP_'+year+'.csv',index=False)
    stats = semopy.calc_stats(model)
    stats.to_csv(cwd+r'Results/'+'Stats_'+optimiser+'_SLSQP_'+year+'.csv',index=False)
    return None

Parallel(n_jobs=4)(delayed(sem_results)(optimiser) for optimiser in ['WLS','MLW','DWLS','FIML','ULS','GLS'])